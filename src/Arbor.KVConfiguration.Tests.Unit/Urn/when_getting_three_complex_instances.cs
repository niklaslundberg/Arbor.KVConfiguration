...
namespace Arbor.KVConfiguration.Tests.Unit.Urn
{
    ...
    public class when_getting_three_complex_instances
    {
        ...
        It should_have_instance1_id1 = () => { instances.OrderBy(i => i.Id).First().Id.ShouldEqual("myId1"); };

        It should_have_instance1_name1 = () => { instances.OrderBy(i => i.Id).First().Name.ShouldEqual("myName1"); };

        It should_have_instance1_uri1 = () => { instances.OrderBy(i => i.Id).First().Uri.ShouldBeNull(); };

        It should_have_instance2_children2 =
            () => { instances.OrderBy(i => i.Id).Skip(1).First().Children.ShouldBeEmpty(); };

        It should_have_instance2_id2 = () => { instances.OrderBy(i => i.Id).Skip(1).First().Id.ShouldEqual("myId2"); };

        It should_have_instance2_name2 =
            () => { instances.OrderBy(i => i.Id).Skip(1).First().Name.ShouldEqual("myName2"); };

        It should_have_instance2_uri2 = () => { instances.OrderBy(i => i.Id).Skip(1).First().Uri.ShouldBeNull(); };
        ...
        It should_have_instance3_id3 = () => { instances.OrderBy(i => i.Id).Skip(2).First().Id.ShouldEqual("myId3"); };
